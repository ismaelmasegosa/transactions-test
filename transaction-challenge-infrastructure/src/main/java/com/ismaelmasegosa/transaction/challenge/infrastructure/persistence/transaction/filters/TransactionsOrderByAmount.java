...
public class TransactionsOrderByAmount implements TransactionFilter {

  private final Filter filter;

  public TransactionsOrderByAmount(Filter filter) {
    this.filter = filter;
  }
  ...
  public List<TransactionEntity> action(TransactionRepository transactionRepository) {
    return transactionRepository.findOrderByAmount(filter.getSort());
  }
}
